...
export async function getUserActivityFeed(req: AuthenticatedRequest, res: Response): Promise<void> {
  try {
    ...
    if (ridesError) {
      console.error('Activity feed rides query error:', ridesError.message, ridesError.code);
    ...
    for (const bRaw of bookings) {
      ...
      if (bRaw.status === 'confirmed' || bRaw.status === 'paid') {
        feed.push({
          ...
          kind: 'booking_paid',
          text: `Payment confirmed for ${route} - N${Number(bRaw.total_amount || 0).toLocaleString()}`,
      ...
      if (bRaw.status === 'completed') {
        feed.push({
          ...
          text: `Rider dropped off - N${Number(bRaw.total_amount || 0).toLocaleString()} earned for ${route}`,
    ...
    try {
      ...
      if (reviewsError) {
        console.error('Activity feed reviews query error:', reviewsError.message, reviewsError.code);
      }
      ...
      console.error('Activity feed reviews query threw:', e);
    }
